...
static u32 inline_counter_type(State* vm) {
  return register_foreign_type(vm, "demo/inline-counter", nullptr);
}
...
static Value counter_payload(State* vm, const char* who, Value value, i64** out) {
  if (value.tag != Tag_Foreign) return raise_error(vm, "%s: expected demo counter", who);
...
static Value nat_release_counter(State* vm, u32 base, u32 argc) {
  ...
  if (ARG(0).tag != Tag_Foreign) return raise_error(vm, "release-counter!: expected demo counter");
...
void register_demo_extension(State* vm) { register_native_module(vm, "demo", init_demo); }
