...
static bool any_float(Vm& vm, u32 base, u32 argc) {
  for (u32 i = 0; i < argc; i++) if (ARG(i).tag == Tag::Float) return true;
  return false;
}
...
Value nat_div(Vm& vm, u32 base, u32 argc) {
  ...
  u32 first = 0;
  Value acc;
  if (argc == 1) { acc = int_v(1); }
  else { acc = ARG(0); first = 1; }
  for (u32 i = first; i < argc; i++) {
    Value b = ARG(i);
    if (acc.tag == Tag::Int && b.tag == Tag::Int) {
      if (b.i == 0) return raise_error(vm, "/: division by zero");
      if (b.i != 0 && a_rem(acc.i, b.i) == 0)
        acc = int_v(idiv_wrap(acc.i, b.i));
      else
        acc = float_v((f64)acc.i / (f64)b.i);
    } else {
...
static Value nat_min(Vm& vm, u32 base, u32 argc) {
  ...
  Value best = ARG(0);
  for (u32 i = 1; i < argc; i++) if (num_cmp(ARG(i), best) < 0) best = ARG(i);
  return best;
}
...
static Value round_like(Vm& vm, u32 base, u32 argc, const char* who,
                        f64 (*op)(f64)) {
  if (argc != 1 || !is_num(ARG(0)))
    return raise_error(vm, "%s: expected one number", who);
  Value v = ARG(0);
...
static Value nat_floor(Vm& vm, u32 base, u32 argc)   { return round_like(vm, base, argc, "floor", op_floor); }
static Value nat_ceiling(Vm& vm, u32 base, u32 argc) { return round_like(vm, base, argc, "ceiling", op_ceil); }
static Value nat_round(Vm& vm, u32 base, u32 argc)   { return round_like(vm, base, argc, "round", op_round); }

// comparison chains
static Value chain(Vm& vm, u32 base, u32 argc, const char* who,
                   bool (*ok)(int cmp)) {
