...
struct TableEntry { u64 hash; Value key; Value val; };
struct TableData {
  u32 count;        // live entries
  u32 tombstones;   // dead entries still in `entries`
  TableEntry* entries; u32 entriesLen; u32 entriesCap;  // insertion order
...
struct Heap {
  ...
  struct RootEntry { RootWalkFn fn; void* ud; };
  ...
  char* toSpace; u32 toSize; u32 toUsed;
...
inline const char*   string_bytes(Value v){ return (const char*)((char*)obj_payload(v.obj) + sizeof(StringData)); }
