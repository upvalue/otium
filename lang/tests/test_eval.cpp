...
TEST_CASE("namespace switching and qualified refs") {
  Vm* vm = mkvm();
  Value r = run(*vm,
    "(ns t.a) (define x 41) (define- hidden 9)"
    "(ns t.b) (define x 1)"
    "(+ t.a/x x)");
  CHECK(is_int(r, 42));
...
TEST_CASE("try catches unwinding conditions but never quit") {
  Vm* vm = mkvm();
  Value r = run(*vm,
    "(try (error \"b\") (catch ((lambda (c) #t) c) :caught))");
  CHECK(r.tag == Tag::Keyword);
  ...
  CHECK(!is_unwind(r2));
  r = run(*vm,
    "(try (unwind-protect (begin (trip!) 1 2) (set! observed 1))"
...
TEST_CASE("dynamic params: defaults, with-params, removal on unwind") {
  ...
  r = run(*vm,
    "(try (with-params ((*w* 5)) (error \"x\")) (catch ((lambda (c) #t) c) (width)))");
  CHECK(is_int(r, 80));
