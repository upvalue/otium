...
bool ipc_send_message(Process *sender, int target_pid) {
  ...
  if (!receiver) {
    error.serialize(KERNEL__IPC_SEND_MESSAGE__PID_NOT_FOUND,
                    "pid %d not found to receive message (sender %d %s)",
                    target_pid, sender->pid, sender->name);
    return false;
  }

  if (receiver->pid == sender->pid) {
    error.serialize(KERNEL__IPC_SEND_MESSAGE__SELF_SEND,
                    "process cannot send message to itself (sender %d %s)",
  ...
  if (receiver->msg_count >= OT_MSG_LIMIT) {
    error.serialize(
        KERNEL__IPC_SEND_MESSAGE__OVERFLOW,
        "receiver process %d has too many messages already (sender %d %s)",
        receiver->pid, sender->pid, sender->name);
    return false;
  }
  ...
  TRACE_IPC(LSOFT, "allocating page for message %d to %d (sender %d %s)",
            msg_idx, receiver->pid, sender->pid, sender->name);

  PageAddr msg_page = receiver->msg_pages[msg_idx].first;

  if (msg_page.is_null()) {
    receiver->msg_pages[msg_idx] =
        process_alloc_mapped_page(receiver, true, true, false);

    msg_page = receiver->msg_pages[msg_idx].first;

    if (msg_page.is_null()) {
      error.serialize(
          KERNEL__INVARIANT_VIOLATION,
          "failed to allocate message page %d (receiver %d %s), sender %d %s",
  ...
  TRACE_IPC(LSOFT, "sent message %d to (receiver %d %s) (sender %d %s)",
            msg_idx, receiver->pid, receiver->name, sender->pid, sender->name);
