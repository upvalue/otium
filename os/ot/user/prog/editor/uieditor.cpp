// uieditor.cpp - Graphical TEVL editor for the OS
...
struct GraphicsEditorBackend : tevl::Backend {
  ...
  tevl::Key translateKeyEvent(uint16_t code, uint8_t flags) {
    tevl::Key key;
    ...
    switch (code) {
    case KEY_UP_CODE:
      key.ext = tevl::ExtendedKey::ARROW_UP;
      return key;
    case KEY_DOWN_CODE:
      key.ext = tevl::ExtendedKey::ARROW_DOWN;
      return key;
    case KEY_LEFT_CODE:
      key.ext = tevl::ExtendedKey::ARROW_LEFT;
      return key;
    case KEY_RIGHT_CODE:
      key.ext = tevl::ExtendedKey::ARROW_RIGHT;
      return key;
    case KEY_BACKSPACE:
      key.ext = tevl::ExtendedKey::BACKSPACE_KEY;
      return key;
    case KEY_ENTER:
      key.ext = tevl::ExtendedKey::ENTER_KEY;
      return key;
    case KEY_ESC:
      key.ext = tevl::ExtendedKey::ESC_KEY;
      return key;
    case KEY_HOME_CODE:
      key.ext = tevl::ExtendedKey::HOME_KEY;
      return key;
    case KEY_END_CODE:
      key.ext = tevl::ExtendedKey::END_KEY;
      return key;
    case KEY_PAGEUP_CODE:
      key.ext = tevl::ExtendedKey::PAGE_UP;
      return key;
    case KEY_PAGEDOWN_CODE:
      key.ext = tevl::ExtendedKey::PAGE_DOWN;
      return key;
    case KEY_DELETE_CODE:
      key.ext = tevl::ExtendedKey::DEL_KEY;
      return key;
    }
  ...
  virtual tevl::EditorErr setup() override;
  virtual void teardown() override;
  virtual void refresh() override;
  virtual void clear() override;
  virtual tevl::Coord getWindowSize() override;
  virtual Result<tevl::Key, tevl::EditorErr> readKey() override;
  virtual void render(const tevl::Editor &ed) override;
...
struct GraphicsEditorStorage : public LocalStorage {
  ...
  GraphicsEditorStorage()
      : backend(this), gfx_client_storage(PID_NONE), kbd_client_storage(PID_NONE),
        gfx_storage(nullptr, 0, 0), frame_manager_storage(60), editor(nullptr), interp(nullptr) {
...
tevl::Coord GraphicsEditorBackend::getWindowSize() {
  ...
  return tevl::Coord{cols, rows};
}

Result<tevl::Key, tevl::EditorErr> GraphicsEditorBackend::readKey() {
  if (!kbd_client) {
    return Result<tevl::Key, tevl::EditorErr>::ok(tevl::Key{});
  }
  ...
  return Result<tevl::Key, tevl::EditorErr>::ok(
      translateKeyEvent((uint16_t)key_data.code, (uint8_t)key_data.flags));
}

void GraphicsEditorBackend::render(const tevl::Editor &ed) {
  if (!gfx) {
    return;
  }
  ...
  } else if (ed.mode == tevl::EditorMode::COMMND) {
    // Draw command prompt
    char cmd_buffer[256];
  ...
  if (ed.mode == tevl::EditorMode::INSERT) {
...
void edit_main() {
  ...
  // Run the editor
  tevl::tevl_main(&s->backend, s->editor, s->interp, &file_path);
