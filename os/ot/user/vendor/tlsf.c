// tlsf.c - this is https://github.com/mattconte/tlsf
// mildly altered to not have libc function dependencies
#define assert(x)                                                              \
  if (!x) {                                                                    \
    oprintf("tlsf: assert failed: %s\n", #x);                                  \
  }

#undef assert
#define assert(x)
#define printf(...) oprintf(__VA_ARGS__)
