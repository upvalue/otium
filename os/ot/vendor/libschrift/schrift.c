...
// Disable file loading for freestanding/embedded builds
// and use ou_* memory functions instead of stdlib
#if defined(OT_ARCH_RISCV) || defined(OT_ARCH_WASM)
#define SFT_NO_FILE_LOADING 1

// Use otium's memory allocator instead of stdlib
extern void *ou_malloc(size_t size);
extern void ou_free(void *ptr);
extern void *ou_realloc(void *ptr, size_t size);

// Debug logging for allocations
extern int oprintf(const char *fmt, ...);

static inline void *sft_malloc_log(size_t size, const char *file, int line) {
  void *ptr = ou_malloc(size);
  // oprintf("[sft] malloc(%u) = %p at %s:%d\n", (unsigned)size, ptr, file, line);
  return ptr;
}

static inline void sft_free_log(void *ptr, const char *file, int line) {
  // oprintf("[sft] free(%p) at %s:%d\n", ptr, file, line);
  ou_free(ptr);
}

static inline void *sft_realloc_log(void *ptr, size_t size, const char *file, int line) {
  void *newptr = ou_realloc(ptr, size);
  // oprintf("[sft] realloc(%p, %u) = %p at %s:%d\n", ptr, (unsigned)size, newptr, file, line);
  return newptr;
}

static inline void *sft_calloc_log(size_t nmemb, size_t size, const char *file, int line) {
  size_t total = nmemb * size;
  void *ptr = ou_malloc(total);
  // oprintf("[sft] calloc(%u, %u) = %p at %s:%d\n", (unsigned)nmemb, (unsigned)size, ptr, file, line);
  if (ptr) {
    memset(ptr, 0, total);
  }
  return ptr;
...
#define malloc(size) sft_malloc_log(size, __FILE__, __LINE__)
#define free(ptr) sft_free_log(ptr, __FILE__, __LINE__)
#define realloc(ptr, size) sft_realloc_log(ptr, size, __FILE__, __LINE__)
#define calloc(nmemb, size) sft_calloc_log(nmemb, size, __FILE__, __LINE__)
...
#if defined(OT_ARCH_RISCV) || defined(OT_ARCH_WASM)
/* Arena allocator C API (defined in arena.cpp) */
extern void *sft_arena_alloc(void *arena, size_t size);
...
struct Outline {
  ...
  void *arena; /* Optional arena for allocations (lib::Arena* cast to void*) */
};
...
static int init_outline(Outline *outl, void *arena);
...
static inline void *outl_realloc(Outline *outl, void *ptr, size_t size) {
  ...
  return ou_realloc(ptr, size);
}

static inline void outl_free(Outline *outl, void *ptr) {
  /* Arena frees are no-ops (freed on reset) */
  if (!outl->arena) {
    ou_free(ptr);
  }
}
#else
/* Non-embedded: use standard malloc/realloc/free */
static inline void *outl_alloc(Outline *outl, size_t size) {
  (void)outl;
  return malloc(size);
}
...
int sft_render(const SFT *sft, SFT_Glyph glyph, SFT_Image image) {
  ...
  if (init_outline(&outl, sft->arena) < 0)
    goto failure;
...
static Point midpoint(Point a, Point b) { return (Point){0.5f * (a.x + b.x), 0.5f * (a.y + b.y)}; }
...
static int init_outline(Outline *outl, void *arena) {
  ...
  outl->numPoints = 0;
  outl->capPoints = 64;
  if (!(outl->points = (Point *)outl_alloc(outl, outl->capPoints * sizeof *outl->points)))
    return -1;
  outl->numCurves = 0;
  outl->capCurves = 64;
  if (!(outl->curves = (Curve *)outl_alloc(outl, outl->capCurves * sizeof *outl->curves)))
    return -1;
  outl->numLines = 0;
  outl->capLines = 64;
  if (!(outl->lines = (Line *)outl_alloc(outl, outl->capLines * sizeof *outl->lines)))
    return -1;
  return 0;
}

static void free_outline(Outline *outl) {
  outl_free(outl, outl->points);
  outl_free(outl, outl->curves);
  outl_free(outl, outl->lines);
}
...
static inline int16_t geti16(SFT_Font *font, uint_fast32_t offset) { return (int_least16_t)getu16(font, offset); }
...
static int glyph_bbox(const SFT *sft, uint_fast32_t outline, int box[4]) {
  ...
  box[0] = (int)floorf(box[0] * xScale + sft->xOffset);
  box[1] = (int)floorf(box[1] * yScale + sft->yOffset);
  box[2] = (int)ceilf(box[2] * xScale + sft->xOffset);
  box[3] = (int)ceilf(box[3] * yScale + sft->yOffset);
  return 0;
}
...
static int outline_offset(SFT_Font *font, SFT_Glyph glyph, uint_fast32_t *offset) {
  ...
  uint_fast32_t base, thisOff, nextOff;
  ...
  if (font->locaFormat == 0) {
    ...
    thisOff = 2U * (uint_fast32_t)getu16(font, base);
    nextOff = 2U * (uint_fast32_t)getu16(font, base + 2);
  } else {
    base = loca + 4 * glyph;
    ...
    thisOff = getu32(font, base);
    nextOff = getu32(font, base + 4);
  }

  *offset = thisOff == nextOff ? 0 : glyf + thisOff;
  return 0;
}
