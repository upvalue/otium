...
__attribute__((naked)) __attribute__((aligned(4))) void otk_c_exception(void) {
...
void otk_c_setup(void) {
  ...
  // Zero out stack and RAM
  memset(__bss, 0, __bss_end - __bss);
  memset(__free_ram, 0, __free_ram_end - __free_ram);

  printf("otk_c_setup begin; exception handler set\n");

  // 1MB to play with
  tlsf = tlsf_create_with_pool(__free_ram, 1024 * 1024);

  printf("otk_c_setup: tlsf pool initialized at %x\n", tlsf);
